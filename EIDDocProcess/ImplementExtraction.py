...
def formatPassportEyeData(mrzData):
    ...
    try:
        ...
        # Get Name and gender
        extractedData[json_names.__EID_NAME__] = mrzData.names + " " + mrzData.surname
        # extractedData[json_names.__PP_GENDER__] = mrzData.sex

        extractedData[json_names.__EID_MRZ_TYPE__] = mrzData.mrz_type

        # Format Passport No:
        extractedData[json_names.__EID_NO__] = str(mrzData.optional1).replace("<","")
        ...
        if mrzData.expiration_date is not None:
            ...
            if ValidDate:
                if expiryDate < datetime.date.today():
                    alertMessage.append(2004) # Code indicating that passport is expired.
                extractedData[json_names.__EID_EXPIRY__] = formatDatetime_Date(expiryDate)
            else:
                extractedData[json_names.__EID_EXPIRY__] = None
        else:
            extractedData[json_names.__EID_EXPIRY__] = None
        ...
        if mrzData.date_of_birth is not None:
            ...
            if ValidDate:
                if birthDate > datetime.date.today():
                    alertMessage.append(2006) # Code indicating that dob is expired.
                extractedData[json_names.__EID_DOB__] = formatDatetime_Date(birthDate)
            else:
                extractedData[json_names.__EID_DOB__] = None
        else:
            extractedData[json_names.__EID_DOB__] = None
        ...
        alertMessage.append(2001) # Code indicating passport details not extracted.
        return extractedData, alertMessage
